...
dat_int %>%
  ...
  mutate(encounter = forcats::fct_recode(interaction_lvl, "First" = "first interaction", "Second" = "second interaction")) %>%
...
In the earlier version of the EWA model [@camerer1999experience], $\phi$ and $\delta$ were time-invariant free parameters. In the self-tuning EWA model [@ho2007self], the values of $\delta$ and $\phi$ are learnt from experience. Over time, $\delta$ is updated as
...
For more details on the self-tuning EWA model, we refer the reader to [@ho2007self]. As in the RL model above, actions are chosen based on a softmax decision rule (Equation \ref{eq:softmax}). 
<!-- 
...
IG and MS conceived and planned the experiment. IG collected the data. IG and MS analysed the data and built the computational models.
